...
// 1. JENIS COUNTS
$jenis_stmt = $conn->prepare("SELECT p.jenis_pelanggaran, COUNT(*) as count $base_where GROUP BY p.jenis_pelanggaran");
...
// 2. STATUS COUNTS
$status_stmt = $conn->prepare("SELECT p.status, COUNT(*) as count $base_where GROUP BY p.status");
...
$detail_stmt = $conn->prepare(
    "SELECT p.id, p.tanggal, p.jenis_pelanggaran, p.deskripsi,
            ...
            (SELECT COALESCE(SUM(pp.poin),0) FROM pelanggaran pp WHERE pp.siswa_id = s.id) AS total_poin
     $base_where ORDER BY p.tanggal DESC, s.nama_lengkap ASC"
);
                ...
                <tr>
                    <td>
                        <table class="dt">
                            ...
                            ?>
                                <tr>
                                    <td><span class="badge" style="background:<?= $js['bg'] ?>;color:<?= $js['text'] ?>;border:1px solid <?= $js['border'] ?>"><?= $j ?></span></td>
                                    <td class="tc"><?= $cnt ?></td>
                                    <td class="tc"><?= $pct ?></td>
                                </tr>
                            <?php endforeach; ?>
                    ...
                    <td>
                        <table class="dt">
                            ...
                            ?>
                                <tr>
                                    <td><span class="badge" style="background:<?= $ss['bg'] ?>;color:<?= $ss['text'] ?>"><?= $s ?></span></td>
                                    <td class="tc"><?= $cnt ?></td>
                                    <td class="tc"><?= $pct ?></td>
                                </tr>
                            <?php endforeach; ?>
