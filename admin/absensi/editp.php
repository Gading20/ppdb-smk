            ...
            <li class="relative group">
                ...
                <ul class="ml-8 mt-2 block">
                    <li><a href="../absensi/index.php"
                            class="block p-2 text-gray-600 hover:text-violet-600 hover:bg-violet-100 rounded-lg">Presensi</a></li>
                    ...
                    <li><a href="../absensi/konseling"
                            class="block p-2 text-gray-600 hover:text-violet-600 hover:bg-violet-100 rounded-lg">Konseling</a></li>
                </ul>
            </li>
            <a href="../siswa/"
                class="flex items-center gap-3 text-gray-600 p-3 rounded-lg hover:bg-violet-100 transition-colors">
            ...
            <li class="relative group">
                ...
                <ul class="ml-8 mt-2 hidden group-hover:block">
                    ...
                    <li><a href="../laporan/pelanggaran"
                            class="block p-2 text-gray-600 hover:text-violet-600 hover:bg-violet-100 rounded-lg">Pelanggaran</a></li>
        ...
        <div class="p-4 lg:p-8">
            <div class="max-w-3xl mx-auto">
                ...
                <div class="flex items-center mb-6">
                    <a href="pelanggaran.php" class="mr-3 p-2 rounded-full hover:bg-gray-100 transition-colors">
                        <i class="fas fa-arrow-left"></i>
                    </a>
                    <div>
                        <h1 class="text-xl md:text-2xl font-bold">Edit Pelanggaran</h1>
                        <p class="text-sm text-gray-500">Ubah data pelanggaran siswa</p>
                    </div>
                </div>

                <?php if ($error): ?>
                    <div class="bg-red-500/10 border border-red-200 text-red-400 rounded-lg p-4 mb-6 flex items-start animate-fade-in">
                ...
                <div class="glass-effect rounded-xl p-4 md:p-5 mb-5 animate-fade-in">
                    <div class="flex items-center gap-4">
                        <img src="../../<?= htmlspecialchars($p['foto_profil'] ?: 'assets/default/avatar.png') ?>"
                            alt="Foto"
                            class="w-14 h-14 rounded-xl object-cover border border-violet-200 flex-shrink-0">
                        <div>
                            <p class="font-semibold text-base"><?= htmlspecialchars($p['nama_lengkap']) ?></p>
                            <p class="text-sm text-gray-500"><?= htmlspecialchars($p['nis']) ?> &middot;
                        ...
                        <span class="ml-auto px-3 py-1 rounded-full text-xs font-medium
                            <?= $p['jenis_pelanggaran'] === 'Berat'
                                ...
                                : ($p['jenis_pelanggaran'] === 'Sedang'
                                    ? 'bg-yellow-50 text-yellow-600 border border-yellow-500/30'
                ...
                <form method="POST" class="space-y-5">
                    ...
                    <div class="glass-effect rounded-xl p-4 md:p-6 animate-fade-in">
                        <h2 class="font-semibold text-base mb-4 flex items-center gap-2">
                            <i class="fas fa-edit text-violet-500"></i> Informasi Pelanggaran
                        </h2>

                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            <!-- Jenis (read-only) -->
                            <div>
                                <label class="block text-sm text-gray-500 mb-1.5">Jenis Pelanggaran</label>
                                <div class="flex gap-2">
                                    <?php foreach (['Ringan', 'Sedang', 'Berat'] as $j):
                                        $active = $p['jenis_pelanggaran'] === $j ? 'active-' . strtolower($j) : '';
                                    ?>
                                        <span class="jenis-btn flex-1 py-2 px-1 rounded-lg border border-gray-300 text-sm text-center text-gray-500 cursor-not-allowed opacity-80 <?= $active ?>">
                                            <?= $j ?>
                                        </span>
                                    <?php endforeach; ?>
                                </div>
                            ...
                            <!-- Poin (read-only) -->
                            <div>
                                <label class="block text-sm text-gray-500 mb-1.5">Poin</label>
                                <input type="number" value="<?= (int) $p['poin'] ?>" readonly
                                    class="w-full bg-gray-50/30 border border-gray-300 rounded-lg px-3 py-2.5 text-gray-500 cursor-not-allowed">
                            </div>
                        </div>

                        <!-- Deskripsi (read-only) -->
                        <div class="mb-4">
                            <label class="block text-sm text-gray-500 mb-1.5">Deskripsi Pelanggaran</label>
                            <textarea rows="2" readonly
                                class="w-full bg-gray-50/30 border border-gray-300 rounded-lg px-3 py-2.5 text-gray-500 cursor-not-allowed resize-none"><?= htmlspecialchars($p['deskripsi']) ?></textarea>
                        </div>

                        <!-- Status -->
                        <div>
                            <label class="block text-sm text-gray-500 mb-1.5">Status Tindak Lanjut</label>
                            <select name="status"
                                class="w-full bg-gray-50/50 border border-gray-300 rounded-lg px-3 py-2.5 text-gray-800 focus:outline-none focus:border-violet-500">
                    ...
                    <div class="glass-effect rounded-xl p-4 md:p-6 animate-fade-in">
                        ...
                        <textarea rows="3" readonly
                            ...
                            class="w-full bg-gray-50/30 border border-gray-300 rounded-lg px-3 py-2.5 text-gray-500 cursor-not-allowed resize-none"><?= htmlspecialchars($p['tindakan'] ?? '') ?></textarea>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 justify-end">
                        ...
                        <button type="submit"
                            class="w-full sm:w-auto px-6 py-2.5 bg-yellow-600 hover:bg-yellow-700 text-gray-800 rounded-lg transition-colors font-medium">
